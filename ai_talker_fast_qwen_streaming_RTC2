...
async def _realtime_pipeline_impl(audio):
    ...
    if max_abs > 1.0:
        # データが大きい場合（例: int16 のまま）は、最大値で割る（または 32768.0 で割る）
        audio_np = audio_np / 32768.0   # int16 → float32 の標準的な方法
    

    # AI発話直後のマイク入力をフィルタする（クールダウン 1.5秒までは厳しめに。）
    threshold = 0.015 #基準

    since_ai = time.time() - LAST_AI_SPEAK_TIME

    print(f"\n[SINCE AI] {since_ai:.2f}s")

    if since_ai < 1.5:
        print(f"\n[FILTER AI VOICE] {since_ai:.2f}s")
        threshold = 0.7
...
class VoiceAIHandler(StreamHandler):
    ...
    async def _monitor_data_channel(self):
        global LAST_AI_SPEAK_TIME
        print("[MONITOR] Started")
        while True:
            # チャネルが閉じられた、または null になったら終了
            if self.channel is None or self.channel.readyState != "open":
                print("[MONITOR] Channel closed, stopping")
                break
            try:
                queued_bytes = self.channel.bufferedAmount
                print("[queued_bytes] ",queued_bytes)
                if queued_bytes > 0:
                    LAST_AI_SPEAK_TIME = time.time() #しゃべり続けているようなら更新
    ...
    def _ensure_monitoring(self):
        """データチャネル監視がまだなら開始する（同期的に）"""
        if self.monitoring_started:
            return
        if self.channel is not None and self.channel.readyState == "open":
            self.monitoring_started = True
            # 非同期待機できないので、バックグラウンドループに投げる
            asyncio.run_coroutine_threadsafe(
                self._monitor_data_channel(),
                background_loop
            )
            print("[VoiceAIHandler] Monitoring started")
